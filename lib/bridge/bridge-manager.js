...
export class BridgeManager {
  ...
  _appendPendingAttachments(entry, target, attachments) {
    ...
    for (const att of attachments) {
      // 立即预下载所有附件类型（CDN 链接可能短时间过期，不能等 flush 再下载）
      const adapter = entry?.adapter;
      if (!att.url && att.platformRef && adapter) {
        if (att.type === "image" && adapter.downloadImage) {
          adapter.downloadImage(att.platformRef, att._messageId)
            .then(buf => { att._prefetched = buf; })
            .catch(err => debugLog()?.warn("bridge", `图片预下载失败: ${err.message}`));
        } else if (adapter.downloadFileByRef) {
          adapter.downloadFileByRef(att.platformRef)
            .then(buf => { att._prefetched = buf; })
            .catch(err => debugLog()?.warn("bridge", `${att.type}预下载失败: ${err.message}`));
        }
      }
      target.push(att);
    }
