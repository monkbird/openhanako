...
export function createTelegramAdapter({ token, agentId, onMessage, onStatus }) {
  // 先创建 bot，但不立即开始轮询：需要先清理 webhook + 验证 token
  let bot = new TelegramBot(token, { polling: false });
  let stopped = false;
  let consecutiveErrors = 0;
  let restartTimer = null;

  // 启动时清理 webhook 并验证 token（_startPolling 被提取为独立函数，供初始化和重启共用）
  async function _startPolling() {
    try {
      // 清理可能残留的 webhook（webhook 与长轮询互斥）
      await bot.deleteWebhook();
    } catch (err) {
      debugLog()?.warn("bridge", `[telegram] deleteWebhook 失败（非致命）: ${err.message}`);
    }
    try {
      const me = await bot.getMe();
      console.log(`[telegram] Bot 已连接: @${me.username || me.first_name}`);
    } catch (err) {
      const msg = `Telegram Bot token 无效或 Bot 无法访问: ${err.message}`;
